...
export async function generateStaticParams() {
  const { docs } = await getAllPosts("fr", 200);
...
export default async function BlogPost({
  ...
  const { slug } = await params;
  const post = await getPost(slug, "fr");
  if (!post) notFound();

  const related = await getRelatedPosts(post, "fr", 3);
  const author =
  ...
  return (
    <main className="container mx-auto max-w-3xl px-6 py-16">
      ...
      <article>
        <header>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
            {author ? (
              <AuthorByline author={author} locale="fr" date={post.createdAt} />
            ) : (
            ...
            <span>·</span>
            <span>{stats.readingTimeMin} min de lecture</span>
      ...
      <RelatedPosts posts={related} locale="fr" />
    </main>
  );
}
