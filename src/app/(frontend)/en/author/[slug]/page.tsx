...
export default async function AuthorPageEn({
  ...
  if (!author) notFound();
  const { docs: posts } = await getPostsByAuthor(author.id, "en", 50);
  ...
  return (
    <main className="container mx-auto max-w-4xl px-6 py-16">
      <JsonLd
        data={[
          personSchema(author, "en"),
          breadcrumbSchema(
            [
              { label: "Home", path: "/" },
              { label: "Authors", path: "/blog" },
      ...
      <section className="mt-12">
        ...
        {posts.length === 0 ? (
          ...
          <div className="mt-6 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {posts.map((p) => (
              <PostCard key={p.id} post={p} locale="en" />
            ))}
          </div>
        )}
      </section>
